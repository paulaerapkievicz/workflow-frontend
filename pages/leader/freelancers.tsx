...
import {
  addFreelancer, AgencyFreelancer, PendingFreelancer,
  getPendingFreelancers, approveFreelancer, rejectFreelancer,
...
function LeaderFreelancersPage() {
  ...
  const load = useCallback(async () => {
    const [fl, cs, pf] = await Promise.all([
      api.get("/freelancers").then((r) => r.data as AgencyFreelancer[]),
      getCategories(),
      getPendingFreelancers().catch(() => []),
    ]);
    setList(fl);
    setCategories(cs);
    setPending(pf);
  ...
  return (
    <>
      <Head><title>Colaboradores | Líder</title></Head>
      <main className={panel.container}>
        ...
        <section className={panel.content}>
          ...
          {pending.length > 0 && (
            <>
              <h2 style={{ fontSize: "1.1rem" }}>Cadastros aguardando aprovação</h2>
              <div style={{ overflowX: "auto" }}>
                <table className={panel.table}>
                  <thead><tr><th>Nome</th><th>E-mail</th><th>Telefone</th><th>Documento</th><th>Ações</th></tr></thead>
                  <tbody>
                    {pending.map((f) => (
                      <tr key={f.id}>
                        <td>{f.name}</td>
