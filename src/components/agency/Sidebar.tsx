...
export default function AgencySidebar() {
  const {
    uniformsToShip, photosToReview, registrationsToApprove, branchesToApprove,
    onboardingDocsToReview, onboardingAsoToUpload, onboardingContractToRelease, onboardingActivationsToConfirm,
    memberCreditsToReview, contestationsToReview, paymentProofsToReview, alertsOpen,
  } = usePendingCounts("agency");
  const onboardingBadge =
    uniformsToShip + photosToReview + registrationsToApprove +
    onboardingDocsToReview + onboardingAsoToUpload + onboardingContractToRelease + onboardingActivationsToConfirm;
  ...
  const items = applySidebarOrder<SideNavItem>(
    ([
      ...
      { href: "/agency/onboarding", label: "Onboarding", icon: "clipboard-edit", badge: onboardingBadge },
